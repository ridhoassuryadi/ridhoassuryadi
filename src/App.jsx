...
// Lazy loaded components (not immediately visible on load)
...
// Import Mac OS 9 override LAST to ensure it overrides all component styles
import './css/MacOS9Override.css'
...
function App() {
  ...
  const [weather, setWeather] = useState(() => {
    ...
    if (storedTempF && storedIconCode) {
      return { temp: JSON.parse(storedTempF), code: parseInt(storedIconCode, 10) }
    }
    return null
  })
  ...
  const [tileBG, setTileBG] = useState('#6E8898')
  ...
  const [resumeStartBar, setResumeStartBar] = useState(false)
  ...
  useEffect(() => {
    ...
    UserCreatedFolderRef.current = UserCreatedFolder.map((_, i) => UserCreatedFolderRef.current[i] || React.createRef())
  }, [UserCreatedFolder])
  ...
  const textError = // error message
    (
      <>
        Cannot find the file '{RunInputVal || regErrorPopUpVal}' (or one of its component). Make sure the path and
  ...
  useEffect(() => {
    // Clear old localStorage data (MSN, Store removed)
  ...
  const connectWebSocket = async () => {
    try {
      // Wake up the Render backend (silently fail if not available)
      const pingResponse = await fetch('https://notebackend-wrqt.onrender.com/ping').catch(() => null)
      if (!pingResponse || !pingResponse.ok) {
        // Backend not available, skip WebSocket connection
        setLoading(false)
        return
      }
  ...
  useEffect(() => {
    // Live chat disabled - skip WebSocket connection
    setLoading(false)
  }, [])
  ...
  const handleOnDrag = (name, ref, _type) => () => {
    setDragging(true)
    const iconRef = ref
  ...
  if (loading) {
    const localThemeBg = localStorage.getItem('theme') || '#6E8898'
  ...
  return (
    <CrtEffect>
      <UserContext.Provider value={contextValue}>
            ...
            <EmptyFolder
              ...
              setState={(newState) => {
                setUserCreatedFolder((prev) => {
                  const updated = prev.map((f) => (f.id === folder.id ? { ...f, ...newState } : f))
        ...
        <EmptyFolder state={PaintExpand} setState={setPaintExpand} folderName="Paint" paintMode={true} />

        <EmptyFolder state={pictureExpand} setState={setPictureExpand} refState={PictureRef} folderName="Picture" />

        <EmptyFolder state={BinExpand} setState={setBinExpand} refState={BinRef} folderName="RecycleBin" />

        <EmptyFolder state={UtilityExpand} setState={setUtilityExpand} refState={UtilityRef} folderName="Utility" />

        <EmptyFolder state={photoOpenExpand} setState={setPhotoOpenExpand} folderName="Photo" photoMode={true} />
        <Suspense fallback={null}>
          <TaskManager />
          <Patch />
          <SpinningCat />
  ...
  function deletepermanently(deleteName) {
    ...
    const findUserCreatedFolder = UserCreatedFolder.find((icon) => icon.name === deleteName)

    if (findUserCreatedFolder) {
      const updatedFolders = UserCreatedFolder.filter((folder) => folder.name !== findUserCreatedFolder.name)
  ...
  function handleDragStop(_data, iconName, ref) {
    ...
    if (iconElement) {
      ...
      setDesktopIcon((prevIcons) => {
        ...
        const updatedIcons = prevIcons.map((icon) => (icon.name === iconName ? { ...icon, x: x, y: y } : icon))
  ...
  function inlineStyleExpand(name) {
    ...
    if (item) {
      return {
        ...
        top: `${(item.usestate.y <= 0 ? Math.abs(item.usestate.y) * 2 + item.usestate.y : -item.usestate.y) + 37}px`,
