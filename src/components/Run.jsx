...
function Run() {
  ...
  function handleRunOpenFile(_ObjectState, name) {
    const lowerCaseName = name.toLowerCase().trim()
    const matchedItem = desktopIcon.find((item) => item.name.toLowerCase().trim() === lowerCaseName)
  ...
  const listItems = desktopIcon
    .filter((item) => {
      ...
      return !cannotOpenFile.includes(lowerCaseName) && lowerCaseName !== 'resumefile' && !lowerCaseName.startsWith('0')
    })
    .map((item) => item.name)
